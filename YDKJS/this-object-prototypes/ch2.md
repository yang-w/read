...
Whatever you call it, the easiest way to set it up as **totally empty** is `Object.create(null)`. `Object.create(null)` is similar to `{ }`, but without the delegation to `Object.prototype`, so it's "more empty" than just `{ }`.
