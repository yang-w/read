...
The `[[Get]]` operation is invoked when you reference a property on an object, such as `myObject.a`. For that default `[[Get]]` operation, the first step is to check if the object itself has a property `a` on it, and if so, it's used.

**Note:** ES6 Proxies are outside of our discussion scope here, but `[[Get]]` and `[[Put]]` behavior described in this chapter does not apply if a `Proxy` is involved.
...
Some utilities found here include `.toString()`, `.valueOf()`, `.hasOwnProperty(..)`, and `.isPrototypeOf(..)`.

### Setting & Shadowing Properties

Setting properties on an object is more nuanced than just adding a new property or changing an existing property's value.
...
This object is often called "Foo's prototype", because we access it via an unfortunately-named `Foo.prototype` property reference. However, that terminology is destined to cause confusion. I'll call it "the object arbitrarily labeled Foo dot prototype".
...
In class-oriented languages, multiple **copies** (aka, "instances") of a class can be made, like stamping something out from a mold. This happens because the process of instantiating (or inheriting from) a class means, "copy the behavior plan from that class into a physical object", and this is done again for each new instance.
...
By convention in the JavaScript world, "class"es are named with a capital letter, so the fact that it's `Foo` instead of `foo` is a strong clue that we intend it to be a "class".

**Note:** This convention is so strong that many JS linters actually *complain* if you call `new` on a method with a lowercase name, or if we don't call `new` on a function that happens to start with a capital letter. Even though the capital letter doesn't mean ***anything* at all** to the JS engine.
...
By virtue of the `[[Prototype]]` link, and how it provides fall-back look-up steps if a property reference isn't found directly on an object, as part of the default `[[Get]]` algorithm:
...
**Note:** In ES6, an advanced functionality called `Proxy` is introduced which can provide something of a "method not found" type of behavior.
...
In other words, delegation will tend to be less surprising/confusing if it's an internal implementation detail rather than plainly exposed in your API design.
