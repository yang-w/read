...
**Note:** The ES6 syntax for indicating a pause point in code is `yield` -- suggesting a politely *cooperative* yielding of control.
...
Let's walk through the behavior flow:
...
Now let's experiment to see the effects of interleaving these different chunks of `*foo()` and `*bar()`. We'll start with the boring base case, making sure `*foo()` totally finishes before `*bar()`:
...
The end result is `11` and `22`. Now let's mix up the interleaving ordering and see how it changes the final values of `a` and `b`:
...
In the previous section, we mentioned an interesting use for generators, as a way to produce values. This use case is essentially the origin of the name: generators.
...
Let's see how generators apply to async coding patterns. Here's a callback-based approach to an Ajax request:
...
The generator approach is actually better. Let's look at the important parts:
...
We're calling a normal function `foo(..)` and apparently getting back the `text` from the Ajax call, even though it's asynchronous. The `yield` used in a generator is the key -- it allows apparently blocking, synchronous-looking code that only pauses/blocks the code in the generator itself, not the whole program.
...
We have totally synchronous-looking code inside the generator (other than the `yield` keyword itself), but hidden behind the scenes, inside of `foo(..)`, the operations can complete asynchronously. We are abstracting the asynchrony away as an implementation detail, so that we can reason synchronously/sequentially about our flow control: "Make an Ajax request, and when it finishes print out the response." This same capability extends without bounds, to let us express however many steps we need to.
...
Generators iterated asynchronously are a huge step forward in sequential reason-ability, but we lose the trustability and composability of Promises. The best of all worlds in ES6 is to combine generators (synchronous-looking async code) with Promises (trustable and composable).

Here's the Promise-based approach to the same Ajax example:
...
Several Promise abstraction libraries provide a utility for running Promise-`yield`ing generators. For learning purposes, let's define our own standalone utility called `run(..)`:
...
**Note:** We can use ES6 destructuring assignment to simplify the `var r1 = .. var r2 = ..` assignments, with `var [r1,r2] = results`.
...
Two simultaneously running "processes" can cooperatively interleave their operations, enabling powerful asynchrony expressions.

Recall a scenario where two different simultaneous Ajax response handlers need to coordinate to prevent a race condition. We slotted the responses into the `res` array like this:
...
`yield`ing a Promise from a generator is the best way to manage asynchrony with generators, but there's another pattern with some adoption worth briefly examining.
...
Instead of `thunkify(..)` making the thunk itself, typically the `thunkify(..)` utility would produce a function that produces thunks.
