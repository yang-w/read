...
In fact, most new async APIs being added to JS/DOM platform are being built on Promises.
...
Let's look at two different analogies for what a Promise *is*.
...
There's something very important to notice about this async pattern.
...
Setting aside the fact that this logic is a bit ugly to implement in various places, there's something deeper and more troubling going on.
...
Primarily, this is a concern of whether code can introduce Zalgo-like effects, where sometimes a task finishes synchronously and sometimes asynchronously, which can lead to race conditions.
